...
test("el dueño configura el RIF y la serie fiscal", async ({ page }) => {
  ...
  if (await locked.isVisible()) {
    // Ya se emitió con él: cambiarlo contradiría las facturas que están en la
    // calle, y la pantalla tiene que decirlo en vez de ofrecer un campo que
    // sólo puede responder 409.
    await expect(locked).toContainText(/J/);
  } else {
    ...
    // Sobrevive a la recarga: es la diferencia entre guardado y pintado.
    //
    // Se compara sin puntuación: se escribe `J383479583` y vuelve
    // `J-38347958-3`, porque la pantalla lo presenta con guiones. Es el mismo
    // número, y exigir la misma cadena sería exigir que no se formatee.
  ...
  const expected = `00-${first.padStart(8, "0")}`;
  ...
  await expect(page.getByTestId("fiscal-series-next")).toContainText(expected);
});
